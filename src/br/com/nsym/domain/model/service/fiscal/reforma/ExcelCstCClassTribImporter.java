...
public class ExcelCstCClassTribImporter {
    ...
    private int importarCCredPres(Sheet sheet) throws Exception {
        ...
        for (int r = 1; r <= lastRow; r++) {
            ...
            if (isBlank(codigo)) {
                // linha vazia ou sem código → ignora
                continue;
            }
            ...
            // Descrição
            String desc = str(row, idx.colAny("Descrição", "Descricao"));
            ent.setDescricao(corta(desc,1000));

            // dispositivo legal – coluna "LC 214/2025"
            String lc214 = str(row, idx.col("LC 214/2025"));
            ent.setLc214(corta(lc214, 1000));

            // percentual de crédito presumido – usar "pAliqCredPresCBS"
            ent.setPAliq(dec(row, idx.colAny("pAliqCredPresCBS", "pAliq")));

            // *** Estes campos não existem na planilha atual, então deixam nulos ***
            ...
            ent.setAliqCbs(dec(row, idx.col("Alíquota CBS")));
            ent.setAliqIbs(dec(row, idx.col("Alíquota IBS")));
